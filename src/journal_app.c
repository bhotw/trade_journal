...
static void journal_app_startup(GApplication *app) {
  ...
  // Set keyboard shortcut for quitting
  gtk_application_set_accels_for_action(GTK_APPLICATION(app), "app.quit", quit_accels);
}

static void journal_app_activate(GApplication *app) {
...
static void journal_app_open(GApplication *app, GFile **files, int n_files, const char *hint) {
  GList *windows = gtk_application_get_windows(GTK_APPLICATION(app));
  JournalAppWindow *win;
  int i;
...
static void journal_app_class_init(JournalAppClass *class) {
